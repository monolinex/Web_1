...
// Track button clicks (e.g., Home, About, Contact)
function trackClick(tag) {
  gtag('event', 'button_click_type', {
    'button_click_type': `${tag}_clicked`
  });
}
...
if (video) {
  video.addEventListener("play", () => {
    gtag('event', 'video_play_count', {
      'video_play_count': 1
    });
    gtag('event', 'video_play_status', {
      'video_play_status': 'played'
    });
  });
  video.addEventListener("pause", () => {
    gtag('event', 'video_play_status', {
...
formInputs.forEach(input => {
  input.addEventListener("focus", () => {
    gtag('event', 'entered_form_field', {
      'entered_form_field': input.name
    });
  });
});
